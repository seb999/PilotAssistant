...
static const float UI_SMOOTH = 0.08f;  // lower = snappier (was 0.15f)
...
GyroRange gyro_range = GYRO_RANGE_500DPS;
AccelRange accel_range = ACCEL_RANGE_4G;
...
// ===================== STATUS DISPLAY =====================
void lcd_status(const char* line1, const char* line2) {
...
void draw_attitude_indicator(float roll, float pitch, float heading) {
    ...
    // Flush framebuffer to LCD (single DMA transfer)
    lcd_flush();
}
